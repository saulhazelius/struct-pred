...
def input_data(in_file):
	...
	X_trans = transformer.fit_transform(np.array(X_in))		
...
class Net(nn.Module):
	def __init__(self):
		...
		self.f1 = nn.Linear(10,1200) #input to hidden layer 
		self.l1 = nn.Tanh()
		self.f2 = nn.Linear(1200,20*unit)# hidden to output layer
	def forward(self,x):
		x = self.f1(x)	
		x = self.l1(x)
		x = self.f2(x)
...
embed_size = 20*unit ### same as final. The feature vector is linearly transformed to have the same dimension as the input dimension of the GRU network. 
hidden_size = 20*unit
num_layers = 1 # GRU 
...
def evaluateTodo(encoder,decoder,ppair):
	for i in range(len(ppair)):
		X = torch.tensor(ppair[i][0])
		smi = ppair[i][1]
...
for k in range(len(hlist)):
	...
	for i in range(epochs):
		...
		evaluateTodo(encoder,decoder,val_pairs)
...
#		print(j,voss)
	...
	plt.ylabel('loss')
	plt.savefig('CV'+str(k+1)+'_'+'1200_'+str(hidden_size)+'embhidsize_epchs_'+str(epochs)+'.pdf',format='pdf')
	plt.clf()
	plt.cla()
	plt.close()
